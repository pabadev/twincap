...
- [x] **C12-3 (§4)** Venta POS desktop — ✅ `b484c50` `feat(pos): desktop sale layout`: grid `lg:grid-cols-[1fr_20rem]` en 3 zonas (carrito izquierdo + ajustes arriba-derecha + resumen sticky abajo-derecha con total/acciones), Modal size `xl` (max-w-5xl) en ambas monturas, mobile intacta (1 columna), lógica `createSale` sin tocar; 6 tests nuevos (19/19 sale-form, 23/23 con provider), tsc/lint/prettier limpio, spot-check vitest 19/19
