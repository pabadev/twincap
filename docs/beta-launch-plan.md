...
- [ ] Índices en Atlas intactos (contrato completo, R15.3.2): `node --env-file=.env.local scripts/verify-all-indexes.mjs` → `CONTRACT OK` (ejecuta en serie auth-token + movement + idempotency + dashboard + monitor, cada uno con su propia conexión Mongoose). Si `verify-auth-token-indexes.mjs` falla, materializar el índice partial unique de auth-token con `node --env-file=.env.local scripts/ensure-auth-token-indexes.mjs --apply` y re-verificar. El deploy NUNCA depende de `autoIndex` (desactivado en producción, R15.2-C1): los scripts ensure/verify son el mecanismo.
