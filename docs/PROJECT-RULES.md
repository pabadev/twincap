...
## 20. Rama y despliegue (REGLA PERMANENTE — fundador, 2026-09-16)

> **La rama del proyecto es `master` y el único proyecto de despliegue es TwinCap (`twincap.vercel.app`).** Estas dos reglas son permanentes y valen para cualquier agente, herramienta o proceso futuro. Razón: la integración legacy `globalmoney` (pre-rename) desplegaba cada PR contra un proyecto Vercel fantasma y hacía fallar el chequeo "Vercel – globalmoney", costando tiempo y recursos.

- **La rama del proyecto es `master`.** NO existe ni se crea una rama `main`. Todo PR, push, workflow de CI, deploy y operación apunta a `master`. Prohibido: renombrar, forkar o crear configuraciones que referencien `main`, cambiar el default branch, o redirigir despliegues a otra rama sin aprobación explícita del fundador.
- **NO desplegar ni configurar NADA bajo el nombre `globalmoney`.** El proyecto Vercel legacy `globalmoney` fue desconectado (2026-09-16: environments `Preview – globalmoney` y `Production – globalmoney` eliminados de GitHub). El único proyecto Vercel es **`twincap`** (`twincap.vercel.app`), vinculado localmente en `.vercel/project.json`.
- **Prohibido introducir procesos de deploy a Vercel o GitHub para `globalmoney`**: workflows de GitHub Actions, `vercel.json`, comandos `vercel` con ese proyecto, scripts de deploy, webhooks, environments o status checks con ese nombre. Cualquier agente que detecte una referencia nueva a `globalmoney` en infraestructura de deploy DEBE reportarla y no ejecutarla.
- **El chequeo "Vercel – globalmoney" no existe como gate válido.** Los checks válidos son CI Quality + E2E (`ci.yml`) y los deployments de `twincap`. Un check/status que mencione `globalmoney` es residuo legacy y NO bloquea merges.
- Los workflows existentes quedan como están: `ci.yml` (quality + e2e, trigger `on: push: branches: [master]` y `pull_request`) e `index-ops.yml` (manual Atlas, sin deploy). No agregar un workflow "deploy" sin aprobación explícita.

## 19. Historial de versiones de este archivo

| Fecha                        | Cambio                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-09-16                   | §20 nueva: regla permanente de rama (`master`, nunca `main`) y despliegue (único proyecto Vercel `twincap`; `globalmoney` legacy desconectado — environments de GitHub eliminados; prohibido configurar deploys para `globalmoney`; los checks `globalmoney` no son gate válido). Origen: decisión del fundador 2026-09-16 por el fallo repetido "Vercel – globalmoney" en los PRs que costaba tiempo y recursos.                                                                                                                                                                                                                                                  |
