...
| UX-10 — Implementación | ✅ **COMPLETA Y ARCHIVADA (2026-09-18)** | H-10/H-14 + DEC-IA-02/03/04/10 + DEC-DS-01/03/06 + FormField D8 (SDD `ux-10-implementation`, commits `b704181..43f7492`) |
| UX-11 — Validación | ✅ **COMPLETA (2026-09-18)** | Parte A medida (TTI/§47/axe/protocolo) + Parte B attestada fundador 5/5, informe `PASSED` (SDD `ux-11-validation`) |
| UX-12 — Polish | ✅ **COMPLETA Y ARCHIVADA (2026-09-18)** | Híbrido DEC-DS-01, re-scan axe 0, H-01..H-19 19/19, §56 declarado — **ETAPA CERRADA** (SDD `ux-12-polish-final`) |
...
### UX-10 — IMPLEMENTACIÓN ✅ COMPLETA Y ARCHIVADA (2026-09-18)

- **Objetivo:** ejecutar el orden del §53 (ver §4) aplicando los sistemas aprobados (UX-3/UX-4).
- **Criterios de aceptación — CERRADOS:**
  - **H-10 cerrado:** clientes (`clients/page.tsx`), secciones de categorías (`categories/page.tsx`) y doble render de `noResults` del catálogo (`catalog-list.tsx`) migrados a `EmptyState`; 4 exclusiones documentadas con rationale (dashboards N1 minimalismo, hint inline de modal) — registro H-10 en `design.md` + comentarios `H-10 EXCLUSION (UX-10):` en sitio.
  - **H-14 cerrado:** `BackButton` eliminado de las 5 listas raíz (sales, catalog, payables, credits received/granted); `back-button.tsx` queda sin importadores (borrado diferido a UX-12).
  - **DEC-IA-02/03/04/10 ejecutadas:** nav 4 niveles con `NAV_GROUPS` + group headers (`nav.tsx`), renames "Productos"/"Ventas", grupo "Configuración" con Perfil/Comentarios/Ayuda, `/help` linkeado standalone (ruta verificada self-contained); DEC-IA-11 ya estaba cerrada.
  - **DEC-DS-01/03 ejecutadas:** `text-red-600`→`text-danger` (transfer-form), `hover:bg-indigo-50`→token (button inverse); **D6 sub-set local AA** (Button success `bg-teal-700`, Toast info `bg-blue-700`, Badge texto neutro sobre tintes /10, modal close zinc-400→zinc-500) — corrección global `--tc-*` diferida a UX-12 (decisión fundador).
  - **DEC-DS-06 ejecutada:** componente `Alert` (danger/info) extrayendo 4 banners + verify-banner; ErrorState NO creado (umbral 2+ no alcanzado, diferimiento documentado).
  - **D7 registrado:** creación de venta POS SIN confirmación informada — decisión documentada con rationale (parity con creación de movimiento/cuenta; delete y cobro conservan sus confirmaciones).
  - **D8 ejecutado:** `FormField` extraído (`src/components/ui/form-field.tsx`) con wiring a11y centralizado (htmlFor/aria-describedby/aria-invalid); 6 sitios migrados (5 en sale-form — corregía labels sin asociar, bug a11y genuino — + feedback-widget); filter-bar labels (~21) diferidos UX-12.
  - Pasos 6/10/11 confirmados cerrados en árbol (cero trabajo; `/reports` NO creada — DEC-IA-09).
- **Evidencia:** SDD `ux-10-implementation` — verify **PASS** (0 CRITICAL): **37/37 requirements, 66/66 escenarios COMPLIANT** (57 con evidencia automatizada/estructural; 9 visuales/AT consolidados en tarea 9.5 MANUAL), suite **151 archivos / 1568 tests** (~23 min, timeout 2.9M ms), tsc/lint/build/parity 0, freeze CERO (`git diff` vacío en `src/core/`, `src/infrastructure/`, `globals.css`); 9 commits convencionales C1–C9 + archive `81c5522`; ≈1.219 líneas reales vs 1.300 forecast (size:exception aprobado, PR único); archive-report en `openspec/changes/archive/2026-09-18-ux-10-implementation/`.
...
### UX-11 — VALIDACIÓN ✅ COMPLETA (2026-09-18)

- **Objetivo:** evidencia real, no "el agente terminó" (§48).
- **PARTE A (evidencia medida, SDD `ux-11-validation`, 9 commits `130ae2b..c11bdc5`, 2.903 líneas docs/harness, cero diff de producto, size:exception PR único):**
  ...
  - **(3) Auditoría a11y automatizada:** 1 devDependency nueva `@axe-core/playwright` (aprobada fundador, única de la fase); escaneo autenticado 20/20 combos (5 rutas × 375+1280 × light+dark), 0 gaps → 50 violaciones / 4 reglas (color-contrast 18, aria-prohibited-attr 16, label 8, select-name 8).
  - **(1) Protocolo de usabilidad:** `docs/UX-11-USABILITY-PROTOCOL.md` — 4 tareas en español neutro (venta/gasto/Resumen 10s §41/cobro), receta de datos, guía de observación, criterios por tarea, regla 4/5.
- **PARTE B — CERRADA POR ATTESTACIÓN DEL FUNDADOR (2026-09-18):** 5/5 usuarios pasaron las 4 tareas (Regla de Oro §41 **5/5 ≥ 4/5**); S8.3/S12.2/S13.1 pasadas; pases visuales 9.5/RSL-8/filter-zero pasados; los **12 P1 de axe adjudicados formalmente: ACEPTADOS → backlog UX-12** (0 P0). Nivel de evidencia registrado honestamente en el informe: Parte A medida y reproducible; Parte B attestación del fundador con fecha (capturas granulares por tarea no retenidas).
- **Criterios de aceptación — CUMPLIDOS:** sin P0/P1 abiertos (adjudicados) ✅; Regla de Oro 5/5 ✅; regresión CERO ✅ (gates Parte A). Veredicto del informe: **PASSED**.
- **Fases destinatarias:** UX-12 (primeros ítems: los 12 P1 axe + backlog heredado: back-button.tsx muerto, banners (auth) → Alert, `--tc-*` global, filter-bar labels).

### UX-12 — POLISH FINAL ✅ COMPLETA Y ARCHIVADA (2026-09-18) — CIERRA LA ETAPA UX/UI

- **Objetivo:** refinamiento (micro-interacciones, transiciones, detalles de empty states, P3 residuales H-08/H-19 terminados).
- **Criterios de aceptación — CERRADOS:**
  - **DEC-DS-01 HÍBRIDO ejecutado (hybrid: tokens + sitios):** light primary `#1447E6` (card **5.75**), dark `--tc-primary` `#0069F5` (white **4.84** — cerraba CC-6 con cero diff de color en `button.tsx`), hovers, `--tc-surface-muted` light (6.51), expense `#B91C1C` (5.45), income `#05523B` (7.77); brand-gold/teal conservados (uso solo wordmark; CC-4 por-sitio cyan-800/amber-800); `--tc-*` verificados con pre-check de ratios (`evidence/contrast-ratio-precheck.md`) y **re-scan axe como árbitro**.
  - **a11y estructural:** `aria-label` prohibido fuera del live-region del ToastPanel; **18 labels sin asociar + 4 selects desnudos** de las 5 listas asociados (sale/credits×2/payables/transfers — cierra el diferido D8 de UX-10); **RE-SCAN AXE FINAL: 0 violaciones totales (0 × 4 reglas × 20/20 combos)** tras 3 iteraciones acotadas.
  - **Limpieza:** `back-button.tsx` borrado (H-14 acto 2, test de lectura actualizado mismo commit); `Alert` gana variante `success`; 4 banners danger ((auth) ×3 + feedback) y 2 success (forgot/reset) migrados, copy byte-idéntico (messages diff vacío, parity 4/4).
  - **Polish acotado:** `focus:ring-2` → patrón `focus-visible` (DS línea 164) en button/toast/global-movement-provider/not-found; addendum DS §2.3 de motion (3 líneas, documentando transiciones existentes — CERO animaciones nuevas, §56 prohíbe efectismo). H-08/H-19 verificados cerrados en árbol (cero trabajo).
  - **Cierre de etapa:** tabla final **H-01..H-19: 19/19 CERRADOS** (H-14/H-16 acto 2 aquí) en `docs/UX-12-VALIDATION-REPORT.md`; **declaración §56 ejecutada con evidencia** (Regla de Oro UX-11 5/5, re-scan JSON, gates, TTI UX-11). Residuales documentados: dark `text-primary` links ≈4.0:1 fuera de rutas escaneadas (política de triage por-sitio); caveat attestación UX-11.
- **Evidencia:** SDD `ux-12-polish-final` — verify **PASS** (0 CRITICAL; 1 warning de redacción corregido en archive): gates frescos **tsc 0 / lint 0 err / parity 4/4 / build OK / suite 153 archivos / 1577 tests** (24m50s, timeout 3.0M ms), freeze CERO; 54 archivos +2,026/−236; 10 commits (`e538341..49c0d8d`) + archive `a17a832`; specs canónicas sincronizadas (3 modificadas + 2 nuevas; requisito stale "deferred to UX-12" eliminado por supersedencia). Archive-report en `openspec/changes/archive/2026-09-18-ux-12-polish-final/`.
- **FIN DE LA ETAPA UX/UI (2026-09-13 → 2026-09-18):** freeze → 12 fases → §56 declarado. Post-etapa: roadmap post-beta (Compras NO existe — no inventarlo; Teams/Billing post-beta).
...
| UX-11 | ✅ | Validación — Parte A medida (TTI/§47/axe/protocolo) + Parte B attestada por fundador 5/5 (informe `PASSED`) | Regla de Oro 5/5; 12 P1 adjudicados → UX-12; regresión 0 |
| UX-12 | ✅ | Polish final — híbrido DEC-DS-01, a11y estructural, limpieza, §56 (C `e538341..49c0d8d` + `a17a832`) | Re-scan axe 0 violaciones; H-01..H-19 19/19; §56 declarado — ETAPA CERRADA |
