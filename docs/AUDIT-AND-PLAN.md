...
2. **A**: ✅ nada con moneda ≠ moneda de la cuenta persiste ni se lee (movements, transfers, créditos, payables, ventas, abonos con cuenta de pago validada); catálogo almacena `currency` (con guard para docs legacy sin backfill); venta valida catálogo/cuenta/venta; créditos/payables con moneda inmutable; tests adversariales de moneda (§25-A) en verde — suite **958/958**, tsc/lint/build EXIT 0.
