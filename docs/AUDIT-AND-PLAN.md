...
| **B — Recuperación de cuenta + verificación de email** | **B1 forgot/reset password**: flujo "olvidé mi contraseña" → email con token firmado de un solo uso (expiración, protección anti-abuso reutilizando rate limiter R12-A1), página de reset → nueva contraseña. **B2 verificación de email** (recomendada, no bloqueante para beta privada): token de verificación al registrarse, estado `emailVerified` en el usuario, aviso en la UI. Depende de un servicio de email transaccional (nueva dependencia documentada y justificada — máx 1 nueva por fase). | P0-d, P0-e | A (para gate de calidad) |
