...
| **G — Analítica de producto (activation/retention/usage)** | Instrumentar eventos mínimos sin PII: `register`, `firstLogin`, `accountCreated`, `firstMovement`, `dashboardViewed`, `saleCreated`, `returned`. Computar activation (≥3 movimientos en primeros 2 días), retention 7d/30d, usage (movimientos/usuario). Backend de analítica (puede reutilizar el patrón `OperationLog`/`withAudit` o una colección `AnalyticsEvent`), sin depender de terceros si se prefiere. La pregunta "¿pagarías por seguir usándolo?" se instrumenta como encuesta post-beta. | P2-a | E, F (necesita workspaceId en eventos) |
