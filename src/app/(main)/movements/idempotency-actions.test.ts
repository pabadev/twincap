...
// R15.3.1 §21 — createMovementAction post-commit-failure idempotency against a
...
// mutation commits, then the first revalidate call rejects. The key must stay
...
describe('createMovementAction idempotency — real Mongo (§21 R15.3.1)', () => {
  ...
  beforeAll(async () => {
    ...
    await mongoose.connect(mongod.getUri('twincap_1531_movements'));
  ...
  function createMovementFormData(key: string): FormData {
  ...
  it('§21 R15.3.1: post-commit revalidatePath failure never releases the key — retry replays as duplicate and commits NOTHING', async () => {
    const fd = createMovementFormData('key-mov-post-commit-failure');
    ...
    const first = await createMovementAction(null, fd);
    ...
    // (a) the mutation committed exactly once (one movement)
    expect(await MovementModel.countDocuments({ workspaceId: WS })).toBe(1);
    expect(
      await OperationLogModel.countDocuments({ userId: USER_ID, action: 'createMovement', result: 'success' }),
    ).toBe(1);
    // (d) the claim was NOT released — the key stays consumed
    expect(await IdempotencyModel.countDocuments({ userId: USER_ID, action: 'createMovement' })).toBe(1);

    // (c) retry with the SAME key → deterministic duplicate, commits NOTHING
    const retry = await createMovementAction(null, fd);
    expect(retry).toEqual({ error: 'error.duplicateRequest' });
    expect(await MovementModel.countDocuments({ workspaceId: WS })).toBe(1);
    expect(
      await OperationLogModel.countDocuments({ userId: USER_ID, action: 'createMovement', result: 'duplicate' }),
    ).toBe(1);
  });
});