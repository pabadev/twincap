...
function makeAccount(id: string, currency: Currency = "COP"): Account {
  return new Account({
    id,
    workspaceId: "user-1",
    name: `Account ${id}`,
...
function fakeCreditRepo(
  ...
  created: CreditGranted[];
  updated: CreditGranted[];
  deleted: string[];
  abonosAdded: { creditId: string; abono: AbonoRecord }[];
  abonosEdited: {
    creditId: string;
    abonoId: string;
    updates: Partial<{
  ...
  abonosDeleted: { creditId: string; abonoId: string }[];
  writtenOff: {
    workspaceId: string;
    creditId: string;
  ...
  return {
    ...
    addAbono: vi
      .fn()
      .mockImplementation(async (_userId: string, creditId: string, abono: AbonoRecord) => {
        abonosAdded.push({ creditId, abono });
      }),
    editAbono: vi.fn().mockImplementation(
      async (
        _userId: string,
        creditId: string,
        abonoId: string,
    ...
    deleteAbono: vi
      .fn()
      .mockImplementation(async (_userId: string, creditId: string, abonoId: string) => {
    ...
    markWrittenOff: vi
      ...
      .mockImplementation(
        async (
          workspaceId: string,
          creditId: string,
          marker: { date: Date; movementId: string },
        ) => {
...
describe("deleteAbono", () => {
  ...
  it("deletes the linked movement BEFORE pulling the abono (R5-B atomicity)", async () => {
    ...
    await deleteAbono("user-1", "cg-1", "ab-1", creditRepo, movementRepo, accountRepo, fakeUow());
  ...
  it("propagates non-NotFound movement errors WITHOUT pulling the abono (R5-B)", async () => {
    ...
    await expect(
      deleteAbono("user-1", "cg-1", "ab-1", creditRepo, movementRepo, accountRepo, fakeUow()),
  ...
  it("throws NotFoundError when credit does not exist", async () => {
    ...
    await expect(
      deleteAbono("user-1", "missing", "ab-1", creditRepo, movementRepo, accountRepo, fakeUow()),
    ).rejects.toThrow(NotFoundError);
  });
...
describe("deleteCreditGranted", () => {
  ...
  it("cascade-deletes all linked movements via deleteByRefId then the credit", async () => {
    ...
    await deleteCreditGranted("user-1", "cg-1", creditRepo, movementRepo, accountRepo, fakeUow());
    ...
    // R15.3.2 Fase 4: the accounts affected by the cascade are touched as the
    // last write (credit account = abono account here, deduped to a single touch).
  ...
  it("blocks deletion of a sale-born credit and deletes nothing (R5-D0c)", async () => {
    ...
    await expect(
      deleteCreditGranted("user-1", "cg-1", creditRepo, movementRepo, accountRepo, fakeUow()),
    ).rejects.toThrow(ConflictError);
  ...
  it("throws NotFoundError when credit does not exist", async () => {
    ...
    await expect(
      deleteCreditGranted("user-1", "missing", creditRepo, movementRepo, accountRepo, fakeUow()),
    ).rejects.toThrow(NotFoundError);
    expect(movementRepo.deleteByRefId).not.toHaveBeenCalled();
...
describe("addAbono — capital/interest split (R9)", () => {
  ...
  it("second abono splits into capital recovery + realized interest (2 movements)", async () => {
    const credit = makeCredit({ installments: 2, installmentValue: new Money(55000, "COP") }, [
      {
        id: "ab-1",
        amount: new Money(55000, "COP"),
        date: new Date("2025-07-01"),
        accountId: "acc-1",
        movementId: "m-ab1",
        capitalAmount: new Money(55000, "COP"),
      },
    ]);
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
    ...
    const credit = makeCredit({ installments: 2, installmentValue: new Money(60000, "COP") }, [
      {
        ...
        accountId: "acc-1",
        movementId: "m-ab1",
        capitalAmount: new Money(100000, "COP"),
      },
    ]);
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
...
describe("editAbono — split synchronization (R9)", () => {
  ...
  it("resyncs both movements when an amount edit shrinks a split abono", async () => {
    const credit = makeCredit({ installments: 2, installmentValue: new Money(65000, "COP") }, [
      {
        id: "ab-1",
        amount: new Money(65000, "COP"),
        date: new Date("2025-07-01"),
        accountId: "acc-1",
        movementId: "m-cap1",
        capitalAmount: new Money(65000, "COP"),
      },
      {
        id: "ab-2",
        amount: new Money(65000, "COP"),
        ...
        accountId: "acc-1",
        movementId: "m-cap2",
        capitalAmount: new Money(35000, "COP"),
        interestAmount: new Money(30000, "COP"),
        interestMovementId: "m-int2",
      },
    ]);
  ...
  it("keeps the primary = interest movement on a 100%-interest abono (no duplicate created)", async () => {
    const credit = makeCredit({ installments: 2, installmentValue: new Money(60000, "COP") }, [
      ...
      },
      {
        ...
        accountId: "acc-1",
        movementId: "m-int",
        interestAmount: new Money(20000, "COP"),
      },
    ]);
    const interestMov = makeMovement({
      id: "m-int",
...
describe("deleteAbono — split-linked movements (R9)", () => {
  ...
  it("deletes interest movement, then primary movement, THEN pulls the abono", async () => {
    ...
    expect(deleteMovementMock.mock.invocationCallOrder[1]).toBeLessThan(
      deleteAbonoMock.mock.invocationCallOrder[0],
    );
  });
...
describe("writeOffCreditGranted", () => {
  it("writes off the full principal when no abonos exist (expense on credit account)", async () => {
    ...
    const result = await writeOffCreditGranted(
      "user-1",
      "cg-1",
      creditRepo,
      movementRepo,
  ...
  it("blocks write-off when the debtor returned ALL capital (no capital loss, interest pending)", async () => {
    const credit = makeCredit({ installments: 2, installmentValue: new Money(60000, "COP") }, [
      ...
      },
      {
        ...
        accountId: "acc-1",
        movementId: "m-ab2",
        interestAmount: new Money(10000, "COP"),
      },
    ]);
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
...
describe("markAsPaid — split on final settlement (R9)", () => {
  ...
  it("posts the final abono to the EXPLICIT account even when it differs from the credit account (H-06)", async () => {
    const credit = makeCredit({ accountId: "acc-1" });
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
    });
    const movementRepo = fakeMovementRepo();
    const accountRepo = fakeAccountRepo([makeAccount("acc-1"), makeAccount("acc-2")]);
    const ids = fakeIdGen();
    ...
    expect(result.pending).toBe(0);
    expect(creditRepo.abonosAdded).toHaveLength(1);
    expect(creditRepo.abonosAdded[0].abono.accountId).toBe("acc-2");
    // The full settlement (no installments) is 100% capital recovery → one
    // capital movement; it must land on the EXPLICIT settlement account.
    expect(movementRepo.created).toHaveLength(1);
    expect(movementRepo.created[0].accountId).toBe("acc-2");
    expect(movementRepo.created[0].link?.kind).toBe("creditGrantedAbono");
  });

  it("rejects with NotFoundError when the settlement account does not exist", async () => {
    const credit = makeCredit();
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
    });
    ...
    expect(creditRepo.abonosAdded).toHaveLength(0);
    expect(movementRepo.created).toHaveLength(0);
  });

  it("rejects with ValidationError when the settlement account currency differs from the credit principal (ACC-1)", async () => {
    const credit = makeCredit();
    const creditRepo = fakeCreditRepo({
      findByWorkspaceId: vi.fn().mockResolvedValue([credit]),
    });
    const movementRepo = fakeMovementRepo();
    const accountRepo = fakeAccountRepo([makeAccount("acc-usd", "USD")]);
    const ids = fakeIdGen();
