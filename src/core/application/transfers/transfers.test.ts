...
describe("createTransfer", () => {
  it("creates a same-currency transfer (TRA-2)", async () => {
    ...
    const bumpOrder = (accountRepo.bumpVersion as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder;
    const movementOrder = (movementRepo.create as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder;
  ...
  it("derives effectiveExchangeRate from both amounts for cross-currency (TRA-3)", async () => {
    ...
    const accountRepo = fakeAccountRepo([makeAccount("acc-src"), makeAccount("acc-dst", "USD")]);
    const ids = fakeIdGen();
...
describe("updateTransfer", () => {
  ...
  it("registers a confirmed negative-balance edit (R15.2 D1)", async () => {
    ...
    const movementRepo = fakeMovementRepo({
      ...
      findById: vi.fn().mockImplementation(async (_userId: string, id: string) => {
        ...
        if (id === "mov-inc")
          return makeMovement({ id: "mov-inc", accountId: "acc-dst", type: "income" });
        return null;
      }),
    });
    const accountRepo = fakeAccountRepo([makeAccount("acc-src"), makeAccount("acc-dst")]);
  ...
  it("rejects a cross-currency edit that omits destinationAmount (rate is derived from both amounts)", async () => {
    ...
    const movementRepo = fakeMovementRepo();
    const accountRepo = fakeAccountRepo([makeAccount("acc-src", "USD"), makeAccount("acc-dst")]);
  ...
  it("passes the transfer version as the CAS expected version (R15.3 §5)", async () => {
    ...
    expect(transferRepo.update).toHaveBeenCalledWith(expect.anything(), expect.anything(), 3);
  });
...
describe("§36 transfer test (R15.2)", () => {
  ...
  function seedBalances(source: number, destination: number) {
    return {
      findByAccountIdForBalance: vi
        .fn()
        .mockImplementation(async (_workspaceId: string, accountId: string) => {
...
describe("updateTransfer currency re-check", () => {
  it("rejects when source account currency no longer matches stored currency", async () => {
    const existing = makeTransfer({
      sourceCurrency: "USD",
      destinationCurrency: "USD",
      sourceAmount: new Money(50000, "USD"),
      destinationAmount: new Money(50000, "USD"),
    });
...
describe("deleteTransfer", () => {
  it("deletes both movements then the transfer, touching both accounts (R15.3.2)", async () => {
    ...
    const moveCallOrder = (movementRepo.delete as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder;
    const transferCallOrder = (transferRepo.delete as ReturnType<typeof vi.fn>).mock
      .invocationCallOrder;
...
describe("createTransfer createdAt tiebreaker (B1)", () => {
  it("income movement createdAt is 1ms after expense movement for deterministic same-day sorting", async () => {
    const source = makeAccount("acc-src");
    const dest = makeAccount("acc-dst");
    const accountRepo = fakeAccountRepo([source, dest]);
    const movementRepo = fakeMovementRepo();
    const transferRepo = fakeTransferRepo();
    ...
    // Income movement is 1ms after expense for deterministic ordering
    expect(income.createdAt.getTime()).toBe(expense.createdAt.getTime() + 1);
  });
});
