...
export async function createCreditReceived(
  ...
  return uow.withTransaction(async (tx) => {
    // R15.3.2 P2-4: mint the credit id and build the entity as the FIRST step
    // of the transaction callback — every execution of the callback (including
    // a transient-error replay) works from fresh ids instead of reusing values
    // minted before the transaction started. Validations R5-D1 run here, still
    // BEFORE any write. Id order: credit → movement → op.
