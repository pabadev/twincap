...
function fakePayableRepo(overrides: Partial<PayableRepository> = {}): PayableRepository & {
  ...
  updated: Payable[];
  deleted: string[];
  abonosAdded: { payableId: string; abono: AbonoRecord }[];
  abonosEdited: {
    payableId: string;
    abonoId: string;
    updates: Partial<{ amount: number; date: Date; movementId: string }>;
  }[];
  abonosDeleted: { payableId: string; abonoId: string }[];
} {
  ...
  return {
    ...
    addAbono: vi
      .fn()
      .mockImplementation(async (_userId: string, payableId: string, abono: AbonoRecord) => {
    ...
    deleteAbono: vi
      .fn()
      .mockImplementation(async (_userId: string, payableId: string, abonoId: string) => {
...
describe("deleteAbono", () => {
  ...
  it("deletes the linked movement BEFORE pulling the abono (R5-B atomicity)", async () => {
    ...
    await deleteAbono("user-1", "pay-1", "ab-1", payableRepo, movementRepo, accountRepo, fakeUow());
  ...
  it("propagates non-NotFound movement errors WITHOUT pulling the abono (R5-B)", async () => {
    ...
    await expect(
      deleteAbono("user-1", "pay-1", "ab-1", payableRepo, movementRepo, accountRepo, fakeUow()),
  ...
  it("throws NotFoundError when payable does not exist", async () => {
    ...
    await expect(
      deleteAbono("user-1", "missing", "ab-1", payableRepo, movementRepo, accountRepo, fakeUow()),
    ).rejects.toThrow(NotFoundError);
  });
...
describe("editTotal", () => {
  ...
  it("passes the payable version as the CAS expected version (R15.3 §6)", async () => {
    ...
    await editTotal("user-1", "pay-1", { total: 120000, currency: "COP" }, payableRepo, fakeUow());

    expect(payableRepo.update).toHaveBeenCalledWith(expect.anything(), expect.anything(), 3);
  });
...
describe("deletePayable", () => {
  ...
  it("cascade-deletes all linked movements via deleteByRefId then the payable", async () => {
    ...
    await deletePayable("user-1", "pay-1", payableRepo, movementRepo, accountRepo, fakeUow());
    ...
    // last write (payable account = abono account here, deduped to a single touch).
  ...
  it("throws NotFoundError when payable does not exist", async () => {
    ...
    await expect(
      deletePayable("user-1", "missing", payableRepo, movementRepo, accountRepo, fakeUow()),
    ).rejects.toThrow(NotFoundError);
