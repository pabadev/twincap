...
const beforeLoanRegister = async (page: Page): Promise<{ email: string; password: string }> => {
  const email = `e2e-${Date.now()}-${Math.random().toString(36).slice(2, 8)}@test.local`;
  await registerUser(page, { email });
  return { email, password: "Password123!" };
};

async function createAccountInUI(page: Page, name: string, initialBalance = "0"): Promise<void> {
...
test.describe("Slice 1 — Auth + Accounts", () => {
  ...
  test("register a new user and land on /dashboard with the seeded Efectivo account", async ({
    ...
    await expect(page.getByRole("heading", { name: "Efectivo" })).toBeVisible();
    await expect(page.getByText("COP", { exact: true }).first()).toBeVisible();
  });

  test("login a registered user works and redirects to /dashboard", async ({ page }) => {
  ...
  test("rate limiter blocks the 4th registration within 15 minutes", async ({ page }) => {
  ...
  test("delete an account (no manual movements) via DeleteAccount confirm", async ({ page }) => {
    await beforeLoanRegister(page);
    ...
    await expect(page.getByText("Billetera Aux", { exact: true })).toHaveCount(0);
  });
});
