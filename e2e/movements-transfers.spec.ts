...
async function createAccountInUI(page: Page, name: string, initialBalance: string): Promise<void> {
...
test.describe("Slice 2 — Movements + Transfers + Payables", () => {
  ...
  test("income increases and expense decreases the account balance", async ({ page }) => {
  ...
  test("editing a movement updates its amount, date and category", async ({ page }) => {
    await freshUser(page);
  ...
  test("deleting a manual movement restores the pre-movement balance", async ({ page }) => {
    await freshUser(page);
  ...
  test("transfer moves money between own accounts but never counts as economic result", async ({
    ...
    await expect(incomeLabel.locator("xpath=following-sibling::p").first()).toContainText(
      /COP\s+0/,
    );

    const expenseLabel = page.getByText("Expenses this month").first();
    await expect(expenseLabel.locator("xpath=following-sibling::p").first()).toContainText(
      /COP\s+0/,
    );
  });
