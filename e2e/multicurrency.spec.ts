...
import { registerUser, connectE2eDb, workspaceIdOf, accountIdOf } from "./helpers";
...
async function createAccountInUI(page: Page, name: string, currency: string): Promise<void> {
...
test.describe("R15.3 §11 — Multimoneda", () => {
  ...
  test("190000 COP → 5000 USD minor derives FX 3,800 COP/USD, persists and reloads", async ({
    ...
    await dialog.getByLabel("From Account").selectOption({ label: "Efectivo (COP)" });
    await dialog.getByLabel("To Account").selectOption({ label: "Dollar Account (USD)" });
    ...
    await expect(dialog.getByText(/Each USD received costs 3,800 COP/)).toBeVisible();
