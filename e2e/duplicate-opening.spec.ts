...
test.describe("R15.3 §11 — Opening duplicado", () => {
  ...
  test("concurrent Set Initial Balance: one wins, one is rejected, one opening in DB", async ({
    ...
    // UX-6: each tab now renders the informed-confirmation ("Confirm initial
    // balance") dialog. Confirm BOTH so the two dispatches still race — the
    // confirmation closes optimistically on confirm, so winner/loser
    // semantics below are unchanged (one opening wins, the other is rejected).
    await confirmMoneyAction(page);
    await confirmMoneyAction(pageB);
