...
Design-token discipline for color usage (DEC-DS-01/03 residue): two raw color literals that escaped tokenization are corrected, plus the founder-approved contrast AA LOCAL SUB-SET (decision D6, 2026-09-18) — Button success variant, Toast info, Badge `/10` tints, and modal close icon `zinc-400→zinc-500`. Superseded 2026-09-18: the global `--tc-*` token correction was initially deferred to UX-12 and has since been EXECUTED by UX-12 under the HYBRID option (DEC-DS-01) — see the "DEC-DS-01 hybrid execution" requirement below; the historical "no global token diffs" sub-set scenarios applied only to the UX-10 delta. External tool verification of contrast is part of the sub-set's acceptance evidence.
